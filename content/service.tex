    ...
    if idxEq currIdx maxIdx then
      ...
      writeContextAux (getNextVaddr contextSaveAddr) nextIdx maxIdx dec_bound
		...
		Cette preuve est axée autour de deux points critiques ; la mise à jour de la partition courante, ainsi que l'écriture du contexte de la partition appelante dans son espace d'adressage. Nous commencerons par passer en revue les propriétés souhaitées ainsi que la méthode utilisée pour chacun des deux points critiques de cette preuve, puis nous verrons comment ces propriétés sont récupérées au fur et à mesure de l'exécution des blocs préliminaires du service.
			...
			\paragraph{Les pages dans lesquelles le service écrit sont accessibles} $ $\\
			...
			Récupéré des préconditions

			\paragraph{Les pages ne font pas partie des pages de configuration de Pip} $ $\\
			$\forall p, p \in~$\texttt{partitionTree}$~s$,\\
			\texttt{contextPage} $\notin$ \texttt{getConfigPages}$~p~s~\wedge$\\\texttt{contextEndPage}$~\notin~$\text{getConfigPages}$~p~s$

			Déduit de \texttt{kernelDataIsolation s} et de la propriété précédente.

			\paragraph{Définition de $s'$}$ $\\
			$\exists~E~:~$\texttt{list}$~($\texttt{paddr}$~*~$\texttt{value)}$,~\text{let}~s'=~$\texttt{\{currentPartition}$~s~;~($\texttt{memory}$~s)~\cup~E$\texttt{\}},\\
			$\forall~e~:~($\texttt{paddr}$~*~$\texttt{value}$),~e~\in~E~\implies~\forall~i$,\\ 
			$e~=~($\texttt{contextPage}$~i)~*~$\texttt{defaultVAddr}$~\lor~e~=~($\texttt{contextEndPage}$~i)~*~$\texttt{defaultVAddr}
