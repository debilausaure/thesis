    ...
    if idxEq currIdx maxIdx then
      ...
      writeContextAux (getNextVaddr contextSaveAddr) nextIdx maxIdx dec_bound
		...
		Cette preuve est axée autour de deux points critiques ; la mise à jour de la partition courante, ainsi que l'écriture du contexte de la partition appelante dans son espace d'adressage. Nous commencerons par passer en revue les propriétés souhaitées ainsi que la méthode utilisée pour chacun des deux points critiques de cette preuve, puis nous verrons comment ces propriétés sont récupérées au fur et à mesure de l'exécution des blocs préliminaires du service.
			...
			\begin{subequations}
			\begin{gather*}
				\text{let}~E~=~\bigcup_{i~=~1}^{\mathtt{contextSize}} e_i,\\
				\exists~n,~0~\leq~n~<~\mathtt{contextSize},\\
				\text{let}~E_n~=~\begin{cases}
					         ~\emptyset~~~~~~~~~~~~~\text{si}~n~=~0\\
						 ~\bigcup_{i~=~1}^n e_i~~~\text{si}~n~>~0\\
				                 \end{cases}\\
				\text{let}~s_n~=~\{~\mathtt{currentPartition}~s~;~(\mathtt{memory}~s)~\cup~E_n~\},\\
				\forall~p,~p~\in~\mathtt{getPartitions}~s\\
				\mathtt{getConfigPages}~p~s~=~\mathtt{getConfigPages}~p~s_n
			\end{gather*}
			...
			On suppose que l'hypothèse de récurrence est vraie au rang $n$ (avec $n~<~\mathtt{contextSize}$), on cherche à prouver que cette hypothèse est vraie au rang $n+1$. On cherche donc à prouver :
			\begin{gather*}
				\forall~p,~p~\in~\mathtt{getPartitions}~s\\
			...
			Ainsi, on en déduit que l'écriture $e_{n+1}$ n'a pas d'influence sur la fonction \texttt{getConfigPages}, et donc que :
			...
			\end{equation}
			\end{subequations}
			En particulier, lorsque $n$ atteint sa valeur maximale $\mathtt{contextSize} - 1$, on a $s_{n+1}~=~s'$.
