...
Tout d'abord, le processeur change de pile, en sauvant temporairement les valeurs utilisateurs du segment de pile \texttt{SS} et de la pile \texttt{ESP}, et en les écrivants sur la pile renseignée dans la nouvelle pile renseignée dans la \emph{TSS}. Le processeur sauve ensuite l'état des registres \texttt{EFLAGS}, du segment de code \texttt{CS} et du pointeur d'instruction \texttt{EIP}. EFLAGS contient entre autres l'état des drapeaux d'overflow, de retenue, de parité, de conditions, mais aussi d'activation des interruptions. Le processeur peut pousser un code d'erreur supplémentaire sur la pile dans le cas de déclenchement de certaines fautes afin de préciser leur cause.
	...
	Enfin, la dernière sous-section décrira les travaux les plus reconnus concernant l'application du raisonnement formel sur les systèmes d'exploitation, en finissant par le proto-noyau développé au sein de l'équipe.
		...
		\subsection{Preuve de programme}
			...
			où :
			\begin{itemize}
				\item $P$ représente les \emph{pré-conditions} qui sont les propriétés sur l'état de la machine supposées prouvables\emph{avant} l'exécution du programme à vérifier - ses \emph{prémisses}.
			    \item $c$ représente le code du programme à exécuter sur la machine et qu'on souhaite vérifier.
			    \item $Q$ représente les \emph{post-conditions} qui sont les propriétés sur l'état de la machine qu'on souhaite pouvoir prouver \emph{après} l'exécution du code $c$ du programme à vérifier.
			\end{itemize}
			Au travers du prisme intuitionniste, on pourrait lire le triplet de Hoare $\{P\}~c~\{Q\}$ comme << Étant donné des propriétés $P$ sur l'état de la machine supposées prouvables avant l'exécution du code $c$ du programme, alors les propriétés $Q$ sur l'état de la machine après exécution de $c$ sont prouvables >>.
			
			Par définition, un triplet de Hoare est \emph{prouvable si et seulement si}, quel que soit l'état du système satisfaisant les propriétés $P$ alors soit $c$ arrête l’exécution du système, soit $c$ fait entrer le système dans un nouvel état dans lesquelles les propriétés $Q$ sont prouvables.
			Si le nouvel état engendré par $c$ viole les \emph{post-conditions} $Q$, alors le triplet $\{P\}~c~\{Q\}$ est contradictoire.
			
			Pour s'assurer que $c$ respecte les propriétés qu'on souhaite garantir sur le système, on commence par décomposer $c$ en parties $c_i$ aussi élémentaires que possible.\\
			...
			La logique de Hoare nous permet alors de décomposer la preuve en séparant ces instructions par des états intermédiaires.
			
			\begin{gather}
			    \{P\}\ c_0\ \{Q_0\}, \{Q_0\}\ c_1\ ...\ \{Q_{n-1}\}\ c_n\ \{Q\}
			\end{gather}
			\end{subequations}
			
			La méthode consiste à avancer en prouvant instruction par instruction que les \emph{pré-conditions} $P$ se propagent dans chaque état intermédiaire.


			La logique de Hoare introduit la notion de \emph{triplet de Hoare}, noté génériquement $\{P\}~c~\{Q\}$. $P$ représente les \emph{préconditions} : l'état de la machine, ainsi que les propriétés ou hypothèses connues avant l'exécution du programme - ses prémisses. $c$ représente le code du programme séquentiel à exécuter sur la machine et sur lequel on voudrait établir une preuve. Enfin, $Q$ représente les \emph{postconditions}, l'état de la mahcine ainsi que les propriétés déduites des préconditions $P$ et de l'exécution du programme $c$. En prenant les règles de la logique constructive, on peut lire le triplet de Hoare $\{P\}~c~\{Q\}$ de la sorte : si la machine se trouve dans l'état $P$ avant les sont prouvables avant l'exécution du programme $c$, alors après l'exécution de $c$, on peut démontrer que la machine sera dans l'état $Q$, et les propriétés et hypothèses inclues dans $Q$ sont prouvables.
			La logique de Hoare stipule que le programme $c$ peut être divisé en une séquences d'opérations élémentaires dont les effets sur l'état de la machine sont connus et pris comme axiomes. Le triplet de Hoare peut alors se réécrire de la manière suivante :
			\begin{subequations}
			\begin{gather}
			\{P\}~c~\{Q\} \\
			...
			On peut alors décomposer le triplet initial ; on peut créer des postconditions et préconditions intermédiaires pour chaque opération élémentaire $c_i$ du programme $c$.
			\begin{gather}
				\{P\}~c_1~\{Q_1\}~\{P_2\}~c_2~\{Q_2\}~\hdots~\{P_n\}~c_n~\{Q\}
			\end{gather}

			Or, si le programme $c$ n'est pas interrompu et qu'il est le seul à s'exécuter sur la machine, la postcondition du triplet d'une instruction élémentaire et la précondition de l'instruction suivante sont égales
			...
			\subsubsection{Langage}
				...
				\paragraph{Représentation du programme}
				\begin{itemize}
					\item{Deep embedding}
