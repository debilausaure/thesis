\begin{tikzpicture}[>=triangle 45,font=\sffamily, every text node part/.style={align=center}, scale=1, every node/.style={transform shape}] {
	...
	\node[draw, fill=white, minimum width=5cm, minimum height=2cm] (Cservices) at (10,0) {Code des services\\(Compilé en C)};
	\draw[->] (services) -- (Cservices) node[midway, above] {Digger} node[midway, below] {$\partial x$};
	\node[draw, fill=white, minimum width=5cm, minimum height=2cm] (functions_model) at (0,-2.5) {Fonctions\\sur l'état};
	...
	%\node[draw, fill=white, minimum width=3cm, minimum height=1cm] (models) at (0,8.45) {\footnotesize{Vérification du n° de sauvegarde du contexte}};
	%\node[draw, fill=white, minimum width=3cm, minimum height=1cm] (child_checks) at (-3, 6.95) {\footnotesize{Récupération partition enfant}};
	%\node[draw, fill=white, minimum width=3cm, minimum height=1cm] (parent_checks) at (3, 6.95) {\footnotesize{Récupération partition parent}};
	%\node[draw, fill=white, minimum width=3cm, minimum height=1cm] (source_checks) at (0, 5.45) {\footnotesize{Récupération VIDT appelant}};
	%\node[draw, fill=white, minimum width=3cm, minimum height=1cm] (target_checks) at (0, 4.40) {\footnotesize{Récupération VIDT appelé}};
	%\node[draw, fill=white, minimum width=3cm, minimum height=1cm] (target_ctx) at (0, 3.35) {\footnotesize{Récupération contexte appelé}};
	%\node[draw, fill=white, minimum width=3cm, minimum height=1.5cm] (save_ctx) at (-3, 1.60) {\footnotesize{Vérification de l'espace de sauvegarde du contexte}\\\footnotesize{Sauvegarde du contexte}};
	%\node[draw, fill=white, minimum width=3cm, minimum height=2cm] (switch_ctx) at (2, -0.65) {\footnotesize{Changement d'espace d'adressage}\\\footnotesize{Mise à jour des structures internes de Pip}\\\footnotesize{Chargement du contexte appelé}};
